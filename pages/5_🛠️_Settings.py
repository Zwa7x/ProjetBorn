...
# -----------------------
# CSS pour ajustements visuels (placer après import streamlit)
# -----------------------
st.markdown(
    """
    <style>
...
# -----------------------
expected_cols = ["Supprimer", "Région", "Acronyme", "Lieux (séparés par ,)"]
if "regions_df" not in st.session_state:
    st.session_state["regions_df"] = settings_to_regions_df(settings)
# garantir colonnes et ordre
df_init = st.session_state["regions_df"]
for c in expected_cols:
    if c not in df_init.columns:
        df_init[c] = False if c == "Supprimer" else ""
df_init = df_init[expected_cols]
# forcer type bool pour Supprimer
try:
    df_init["Supprimer"] = df_init["Supprimer"].astype(bool)
except Exception:
    df_init["Supprimer"] = df_init["Supprimer"].apply(lambda v: bool(v) if pd.notna(v) else False)
...
if data_editor:
    ...
    try:
        if isinstance(edited, pd.DataFrame):
            # normaliser colonnes et ordre
            for c in expected_cols:
                if c not in edited.columns:
                    edited[c] = False if c == "Supprimer" else ""
            edited = edited[expected_cols]
            try:
                edited["Supprimer"] = edited["Supprimer"].astype(bool)
            except Exception:
                edited["Supprimer"] = edited["Supprimer"].apply(lambda v: bool(v) if pd.notna(v) else False)
            st.session_state["regions_df"] = edited
        else:
            df_conv = pd.DataFrame(edited)
            for c in expected_cols:
                if c not in df_conv.columns:
                    df_conv[c] = False if c == "Supprimer" else ""
            df_conv = df_conv[expected_cols]
            try:
                df_conv["Supprimer"] = df_conv["Supprimer"].astype(bool)
            except Exception:
                df_conv["Supprimer"] = df_conv["Supprimer"].apply(lambda v: bool(v) if pd.notna(v) else False)
            st.session_state["regions_df"] = df_conv
    except Exception:
        pass
else:
...
# -----------------------
# Bloc d'édition sous le tableau (titre discret)
# -----------------------
